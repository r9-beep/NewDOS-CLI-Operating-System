...
mod time {
    ...
    pub fn formatted_times_short() -> (String<16>, String<16>) {
        let (mut hour, minute, _second) = read_rtc();
        let gmt = format_time_short("GMT", hour, minute);

        let offset = TZ_OFFSET.load(Ordering::SeqCst) as i16;
        hour = ((hour as i16 + offset + 24) % 24) as u8;
        let local_label = timezone_offset();
        let local = format_time_short(local_label.as_str(), hour, minute);
        (gmt, local)
    }
    ...
    fn format_time_short(label: &str, hour: u8, minute: u8) -> String<16> {
...
mod shell {
    ...
    fn cmd_help() {
        ...
        write_line("  pierre ls        - list files/directories");
        ...
        write_line("  pierre cd        - change directory");
    ...
    fn cmd_cd(name: Option<&str>) {
        let Some(name) = name else {
            write_line("cd requires a directory name.");
            return;
        };
        if name == "/" {
            set_cwd("/");
            write_line("Directory changed.");
            return;
        }
        let fs = unsafe { FS.as_ref().expect("fs not initialized") };
        let found = fs
            .list()
            .iter()
            .any(|entry| matches!(entry.kind, EntryKind::Dir) && entry.name.as_str() == name);
        if found {
            set_cwd(name);
            write_line("Directory changed.");
        } else {
            write_line("Directory not found.");
        }
    }
    ...
    fn cmd_version() {
        write_line("NewDOS Base Kernel v0.1.3");
    }
    ...
    fn cmd_time() {
        let (gmt, local) = crate::time::formatted_times_short();
    ...
    static mut CWD: Option<String<24>> = None;
    ...
    fn set_cwd(path: &str) {
        unsafe {
            ...
            if let Some(cwd) = CWD.as_mut() {
                cwd.clear();
                let _ = cwd.push_str(path);
            }
        }
    }

    fn current_cwd() -> String<24> {
        unsafe {
